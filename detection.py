...
# utility generator function that yields all the points in
# a contour as (x, y) tuples
...
class ArbitraryPlaneDetector:
    ...
    def detect(self, frame, gaussian_kernel=(15, 15), alpha=20,
               beta=0.52, debug=False, viz=True):
        """
        ...
        def combinedCostFunction(*args):
            ...
            def fn(cnt):
                ...
                for weight, subFunc in args:
                    ...
                    print "%s: %8.5f (weight=%.3f)" %
                    (subFunc.__name__, value, weight)
                return total
            return fn
        ...
        while len(approxCurve) < 4 and contours:
            ...
            approxCurve = np.array([pt for pt in approxCurve if tuple(pt)
                                    not in rejects])
