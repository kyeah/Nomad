...
# utility generator function that yields all the points in a
# contour as (x, y) tuples
...
class ArbitraryPlaneDetector:
    ...
    def detect(self, frame, gaussian_kernel=(15, 15), alpha=20, beta=0.52,
               debug=False, viz=True):
        """
        ...
        Arguments:
          ...
          alpha: Minimum distance between two points to consider them
            different features
          beta: Minimum angle between two vectors to consider them
            different sides (default: ~30 degrees)
          debug: Modifies the input frame to draw visual debuggers

        Returns:
          An array of x-y pairs indicating the corners in CW order,
        ...
        if debug:
            for i, contour in enumerate(contours):
                ...
                # draw a circle indicating the center of the contour
                # (easier to identify multiple contours)
        ...
        def midDistCost(contour):
            return avg((dist(x, y, cx, cy) / dim for x, y
                        in pointsFromContour(contour)))
        ...
        # note that the weights are inverted because we're doing a
        # minimization. Large weights imply greater importance.
        def combinedCostFunction(*args):
            def fn(cnt):
                ...
                for weight, subFunc in args:
                    ...
                    print "%s: %8.5f (weight=%.3f)" % (subFunc.__name__,
                                                       value, weight)
                return total
            return fn
        ...
        while len(approxCurve) < 4 and contours:
            ...
            approxCurve = np.array([pt for pt in approxCurve if
                                    tuple(pt) not in rejects])
