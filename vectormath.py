...
def test_coplanar(triplet, beta=0.52):
    ...
    # If the three lines are roughly coplanar, find the longest line and return the middle point.
    if angle < beta or abs(np.pi - angle) < beta:
        distances = map(lambda idx: np.linalg.norm(triplet[(idx+1) % 3] - triplet[(idx+2) % 3]), xrange(3))
...
def approx_quadrilateral(corners, alpha):
    ...
    if len(corners) == 2:
        ...
        if abs(dx) > alpha and abs(dy) > alpha:
            if dx < 0: c2, c1 = c1, c2
            corners = np.vstack([corners, [[c1[0] + dx, c1[1]], [c1[0], c1[1] + dy]]])
